    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if (tool != .scroll && tool != .text) && touches.count == 1 {
            ...
            if canCreateTextBox == true {
                ...
                var newLine = Line(color: ThemeColors(rawValue: UserDefaults.standard.integer(forKey: "defaultTintColor"))?.returnUIColor ?? UIColor.systemBlue, width: 2.0, opacity: 1.0, path: UIBezierPath(), type: .drawing)
